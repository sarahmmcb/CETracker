...
IF NOT EXISTS 
    (SELECT 1  
     FROM master.sys.server_principals
     WHERE name = 'IIS APPPOOL\AuthApi')
BEGIN
    CREATE LOGIN [IIS APPPOOL\AuthApi] FROM WINDOWS;
END
GO
...
BEGIN
    CREATE USER [AUTHAPI_SVCACCT] FOR LOGIN [IIS APPPOOL\AuthApi];
END
GO
...
BEGIN
    ...
    ALTER ROLE [AUTHAPI_EXECROLE] ADD MEMBER [AUTHAPI_SVCACCT];
END
GO
